...
.then(res=>{
    ...
    let todo='';
    data.forEach(list => {
       todo+=`<div id="todos"><p>${list.todo}</p><h5 style="color: red;">Deadline: ${list.deadline}</h5><button style="height:40px;width:100px;border-radius: 15px;background-color: red;color: azure;" id="${list._id}"  onclick="remove(this.id)">Delete</button></div>`
